...
dic_replace={'2025.06.30':'2026.06.30'} #py字典，格式为key：value
dic_year={'2024':'2025'}
dic_strs={'GB 20052-2020':'GB 20052-2024'}
dic_strs_1={''}
...
def document_text_replace(root,file_name,dic_):
    ...
    new_file_name=file_name.replace('CFDS2024','CFDS2025')
...
for root,dirs,files in os.walk('E:\\codeproiect\\py1\\temp1'):
   for file in files:
       # 找出文件中以.doc结尾并且不以~$开头的文件(~S是为了排除临时文件的)
       if file.endswith('.docx') and not file.startswith( '.$'):
          print(file)
          document_text_replace(root,file,dic_replace) 
        
          #document_text_replace_in_table(root,file,dic_strs,1)
